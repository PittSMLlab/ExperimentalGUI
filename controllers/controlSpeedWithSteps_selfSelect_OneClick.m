function [RTOTime, LTOTime, RHSTime, LHSTime, commSendTime, commSendFrame] = controlSpeedWithSteps_selfSelect_OneClick(velL,velR,FzThreshold,profilename,mode,signList,paramComputeFunc,paramCalibFunc,numAudioCountDown)
...
try %So that if something fails, communications are closed properly
    ...
    while ~STOP %only runs if stop button is not pressed
        ...
        if ~isnan(velR(RstepCount)) && ~isnan(velL(LstepCount)) %Not in self-controlled mode, yet.
            ...
            %%
            if (RstepCount<N-1 && ~isnan(velR(RstepCount+1))) && (LstepCount<N-1 && ~isnan(velL(LstepCount+1))) || RFBClicker==1 || LFBClicker==1
