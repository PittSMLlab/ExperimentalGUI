...
if first == 'y' | first == 'Y' | first == 'yes' | first == 'Yes'
    first = true;
elseif first == 'n' | first == 'N' | first == 'no' | first == 'No'
    first = false;
else
    error('You did not specify if you want to remove the first stimulus size from the block');
end
...
if first
    trialData=trialData(trialData.isFirstInBlock~=1,:); %Not sure if I want this
end
