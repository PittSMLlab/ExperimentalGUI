...
while currCond < maxCond
    if ~firstCond
        ...
        elseif strcmp(nextCondButton, 'No')
            ...
            currCond = inputdlg('Which condition to start from (1 for baseline, 5 for control train, 7 for 1st split train, enter the number from the 1st col on the data sheet)? ');
    ...
    switch currCond
        ...
        case {5,6} %ctrl train
            ...
            play(AudioTimeUp);
        case {7,8,9,10,11} %pre train
            handles.popupmenu2.set('Value',14) %NIRS train
            profilename = [profileDir 'PreSplitTrain_' num2str(currCond-6) '.mat'];
            manualLoadProfile([],[],handles,profilename)
